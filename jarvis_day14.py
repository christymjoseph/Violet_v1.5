...
# Suppress NLTK download print/logs
...
import JSON
import getpass
import violet_emotions as viem
    ...
    try:
        ...
        if "feel me" in protocol:
            ...
            while k:
                ...
                user_feels = command("speak how do you feel?.....")
                if user_feels == "None":
                    speak("I couldn't hear anything. Let's stop feel me protocol  here, okay?")
                    break

                if user_feels == "stop":
                    speak(" ok.lets stop feel me protocol")
                    break

                user_feeling = viem.emotion_detector(user_feels)
                print(f"you feel {user_feeling}")
                viem.emotion_speaker(user_feeling)
                time.sleep(2)

                speak("what would you like to say next?")
                print("what would you like to say next?")
                user_feeling_after=command("speak how do you feel now dear?.....")
                if user_feeling_after == "None":
                    speak("I didn’t get that. Let’s end the feel me protocol.")
                    return



                if "stop" in user_feeling_after:
                    speak("well")
                    speak("i hope you will have great days dear")
                    k=False
                    return

                user_emotion_now=viem.emotion_detector(user_feeling_after)
                viem.emotion_speaker(user_emotion_now)
...
try:
    while True:
        print("Warming up emotion engine...")
        viem.emotion_detector("this is just a test to load the emotion system")
        ...
        if result is not None and "violet" in result:
            ...
            while True:#violet command loop runs main portion  of the time
                ...
                commander = command()
                new_intent = intent_detector.intent_detect(commander)
